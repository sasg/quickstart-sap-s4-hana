...
if [ ! -s /root/install/sap-s4hana-aas-inst.sh ]
then
	echo "Download of /root/install/sap-s4hana-aas-inst.sh file not successfull"
	echo "exiting..."
	...
	cd /root/install
	bash -x /root/install/sap-s4hana-aas-inst.sh | tee -a /root/install/sap-s4hana-aas-inst.sh-out.log
	if [ $? -ne 0 ] 
	then
		echo 1
		exit 1
