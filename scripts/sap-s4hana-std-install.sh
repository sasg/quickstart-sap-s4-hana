	...
	then
       ...
       #validate /sapmnt filesystems were created and mounted
        ...
        then
            for devf in `ls /dev/nvme?n?`
            do
                if nvme id-ctrl -v ${devf} | grep sdy
                then
                    SAPMNT_VOLUME=${devf}
                fi
            done
        ...
        cd /dev/disk/by-label; ln -s ${SAPMNT_VOLUME} SAPMNT
...
echo "----- Entering sap-s4hana-std-install.sh -----"
